...
class RetinaNetClassificationHead(nn.Module):
    ...
    def forward(self, x, labels_list=None):
        ...
        for i, features in enumerate(x):
            ...
            if labels is None:
                ...
                theta = torch.acos(torch.clamp(cls_logits, -1.0 + 1e-7, 1.0 - 1e-7))
                one_hot = torch.zeros_like(cls_logits)
                one_hot.scatter_(1, labels.view(-1, 1).long(), 1)
                if self.training:
                    with torch.no_grad():
                        B_avg = torch.where(one_hot < 1, torch.exp(
                            self.s * cls_logits), torch.zeros_like(cls_logits))
...
class RetinaNetHead(nn.Module):
    ...
    def forward(self, x, labels_list=None):
        # type: (List[Tensor], List[Tensor]) -> Dict[str, Tensor]
        return {"cls_logits": self.classification_head(x, labels_list), "bbox_regression": self.regression_head(x)}
...
class RetinaNet(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        labels_list = [targets[i]['labels'] for i in range(len(targets))]
        head_outputs = self.head(features, labels_list)
