...
from . import efficientnet_torch
from . import poolformer
from .backbone_utils import BackboneWithFPN, BackboneWithBiFPN
...
def poolformer_backbone(backbone, pretrained, layers, out_channels=160, baseline='fpn', num_layers=2):
    ...
    if baseline == 'bifpn':
        backbone = BackboneWithBiFPN(plformer, in_channles_list, out_channels, num_layers=num_layers, extra_block=True)
    else:
