...
class AdamG(Optimizer):
    def __init__(self,
                 ...
                 betas: Tuple[float, float] = (0.95, 0.999, 0.95),
                 eps: float = 1e-8,
        ...
        for p in group['params']:
            if p.grad is not None:
                ...
                if len(state) == 0:
                    ...
                    state['exp_avg_r'] = torch.zeros_like(p, memory_format=torch.preserve_format)
                ...
                exp_avg_rs.append(state['exp_avg_r'])
                if group['amsgrad']:
...
AdamG.__doc__ = r"""Implements Adam algorithm.
...
def adamg(params: List[Tensor],
          grads: List[Tensor],
          exp_avgs: List[Tensor],
          exp_avg_rs: List[Tensor],
...
def _single_tensor_adam(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if torch.is_complex(param):
            ...
            exp_avg_r = torch.view_as_real(exp_avg_r)
        ...
        exp_avg_r.lerp_(_s_func(exp_avg_sq), 1 - beta3)
        exp_avg.lerp_(exp_avg_r * grad, 1 - beta1)
...
def _multi_tensor_adam(params: List[Tensor],
    ...
    grouped_tensors = Optimizer._group_tensors_by_device_and_dtype(
        [params, grads, exp_avgs, exp_avg_rs, exp_avg_sqs, max_exp_avg_sqs, state_steps])
    for ((
        device_params,
        device_grads,
        device_exp_avgs,
        ...
        if has_complex:
            if amsgrad:
                _view_as_real(device_params, device_grads, device_exp_avgs, device_exp_avg_rs, device_exp_avg_sqs, device_max_exp_avg_sqs)
            else:
                _view_as_real(device_params, device_grads, device_exp_avgs, device_exp_avg_rs, device_exp_avg_sqs)
        ...
        device_exp_avg_v = torch._foreach_pow(device_exp_avg_sqs, 0.24)
        device_exp_avg_v = torch._foreach_mul(device_exp_avg_v, 0.2)

        torch._foreach_lerp_(device_exp_avg_rs, device_exp_avg_v, 1 - beta3)
        torch._foreach_lerp_(device_exp_avgs, torch._foreach_mul(device_exp_avg_rs, device_grads), 1 - beta1)
        ...
        if capturable:
            ...
            step_size = torch._foreach_minimum(torch._foreach_reciprocal(torch._foreach_pow(device_state_steps, 0.5)), 1)
            torch._foreach_div_(step_size, bias_correction1)
            ...
            step_size = _stack_if_compiling([-1 * min(1, 1 / step.item() ** 0.5) / 1 - beta1 ** _get_value(step) for step in device_state_steps])
